...
export class ASRService {
  ...
  // Transcribe PCM buffer, emitting word-by-word partials
  async transcribe(
    pcm: Int16Array,
    _sessionId: string,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `ASR request failed: ${response.status} ${response.statusText} — ${body}`
      );
    }
    ...
    // Simulate streaming partials with word-by-word emission
    this._simulatePartials(transcript, onPartial);

    return transcript;
  }

  // Encode 16-bit PCM to minimal WAV buffer
  private _encodePcmToWav(pcm: Int16Array): Buffer {
    ...
    buf.writeUInt32LE(
      (WAV_SAMPLE_RATE * WAV_CHANNELS * WAV_BIT_DEPTH) / 8,
      28
    );
  ...
  // Emit words one by one with tiny delay for "typing" effect
  private _simulatePartials(
    transcript: string,
    onPartial: PartialTranscriptCallback
  ): void {
